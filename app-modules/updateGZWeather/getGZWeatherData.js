...
module.exports = async () => {
    ...
    const parseWeather = ($elem) => {
        ...
        const temperature = $elem
            .find('.txt')
            .eq(0)
            .text();
        const [lowestTemperature, highestTemperature] = temperature.split('~').map((item) => item.trim().replace('℃', ''));
    ...
    try {
        ...
        const desc = `
            ...
            气温${tomorrowWeather.temperature}, 最低温比今天${getCompareText(lowestTemperatureDiff)}${Math.abs(lowestTemperatureDiff)}度, 最高温比今天${getCompareText(highestTemperatureDiff)}${Math.abs(
            highestTemperatureDiff
        )}度<br><br>
