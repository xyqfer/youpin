...
module.exports = async ({ offsets = [1] }) => {
    ...
    const list = ['http://bang.dangdang.com/books/newhotsales/01.00.00.00.00.00-24hours-0-0-1-', 'http://bang.dangdang.com/books/newhotsales/01.54.00.00.00.00-24hours-0-0-1-'];
    ...
    try {
        const results = await Promise.mapSeries(offsets, async (page) => {
            try {
                return await Promise.mapSeries(list, async (url) => await parseUrl(`${url}${page}`));
            } catch (err) {
                console.error(err);
                return [];
            }
