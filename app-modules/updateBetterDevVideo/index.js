...
module.exports = async () => {
    ...
    try {
        ...
        const [dbData, videoData] = await Promise.all([
            db.getDbData({
                dbName,
                query: {
                    select: [filterKey],
        ...
        let newData = videoData.filter(({ link }) => !dbData.includes(link));
        ...
        if (newData.length > 0) {
            newData = await Promise.mapSeries(newData, async (item) => {
                try {
                    ...
                    item.cover =
                        info.player_response.videoDetails.thumbnail.thumbnails.reduce(
                            (acc, item) => {
