const ProcessImage = ($, e, c) => {
  ...
  if (img.length > 0) {
    ...
    if (img[0].attribs.src) {
      message = `<figure><img referrerpolicy="no-referrer" alt='${img[0].attribs.alt ? img[0].attribs.alt : ''}' src='https://ibdkopi6vn.avosapps.us/api/v1/proxyimage?url=${img[0].attribs.src.replace(/(?<=\/news\/).*?(?=\/cpsprodpb)/, '600')}'><br><figcaption>`;
    } else {
